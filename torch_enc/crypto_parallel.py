...
from torch_enc.encoding import permutation_by_gcd
...
def decrypt_parallel(enc_img : torch.Tensor, key_hex : str,image_hash : str, device = 'cuda:0',  chunk_m_step = 512, chunk_n_step = 512):
    ...
    # dec_img = permute_by_gcd(enc_img, key_decimal, 100, 'decryption',  device)
    ...
    for chunk_m in range(0, m, chunk_m_step): 
        for chunk_n in range(0, n, chunk_n_step):
            ...
            t0 = time()
            dec_img[chunk_m:end_m_chunk, chunk_n:end_n_chunk] = crypto.decryption(enc_img[chunk_m:end_m_chunk, chunk_n:end_n_chunk],
                                                                                  key_image,
                                                                                  key_decimal,
                                                                                  100,
...
def encrypt_parallel(img : torch.Tensor, key_hex : str, device = 'cuda:0', chunk_m_step = 512, chunk_n_step = 512):
    ...
    # enc_img = permute_by_gcd(enc_img, key_decimal, 100, 'encryption',  device)
